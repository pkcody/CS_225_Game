       ...
       // public Text time;
        ...
        void Start()
        {
            ...
            for (int i = 0; i < numberOfSoldiers; i++)
            {
                ...
                Vector3 randomPos = new Vector3(Random.Range(0f, mapWidth), 0f, Random.Range(0f, mapWidth));
                ...
                GameObject newFriendly = friendlyObj;
                //friendlySoldiers.Add(new PlayerController(newFriendly, mapWidth));
        ...
        void Update()
        {
          //  float startTime = Time.realtimeSinceStartup;
            ...
            {
                if (!spatialPartition)
                    closestFriendly = FindClosestFriendlySlow(enemySoldiers[i]);
                else
                    //The fast version with spatial partition
                    closestFriendly = grid.FindClosestFriendly(enemySoldiers[i]);

                //If we found an enemy
                if (closestFriendly != null)
                {
                    //Change material
                    closestFriendly.soldierMeshRenderer.material = closestEnemyMaterial;

                    closestEnemies.Add(closestFriendly);
        ...
        Soldier FindClosestFriendlySlow(Soldier soldier)
        {
